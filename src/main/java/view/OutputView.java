...
public class OutputView {
    ...
    public void printWellComeMessage(final DateProvider dateProvider) {
        ...
        String wellComeMessageFormat = String.format("\n오늘은 %02d월 %02d일 %s입니다. 기능을 선택해 주세요.", dateProvider.getMonth(),
                dateProvider.getDayOfMonth(), dateProvider.getDayOfWeek().getDescription());
    ...
    public void printAttendanceRecord(final LocalDateTime attendanceDateTime, final AttendanceState attendanceState) {
        String attendanceRecordFormat = String.format("\n%02d월 %02d일 %s %02d:%02d (%s)",
                attendanceDateTime.getMonth().getValue(), attendanceDateTime.getDayOfMonth(),
                Calender.findBy(attendanceDateTime).getDescription(), attendanceDateTime.getHour(),
                attendanceDateTime.getMinute(), attendanceState.getState());
    ...
    public void printUpdateAttendanceRecord(final LocalDateTime beforeAttendance,
        ...
        String updateAttendanceResultFormat = String.format("\n%02d월 %02d일 %s %02d:%02d (%s) -> %02d:%02d (%s) 수정 완료!",
                beforeAttendance.getMonthValue(), beforeAttendance.getDayOfMonth(),
                Calender.findBy(beforeAttendance).getDescription(), beforeAttendance.getHour(),
                beforeAttendance.getMinute(), beforeAttendanceState.getState(), afterAttendance.getHour(),
                afterAttendance.getMinute(), afterAttendanceState.getState());
        ...
        for (Entry<LocalDateTime, AttendanceState> attendance : history.entrySet()) {
            ...
            if (key.getHour() != 0) {
                String historyFormat = String.format("%02d월 %02d일 %s %02d:%02d (%s)", key.getMonthValue(),
                        key.getDayOfMonth(), Calender.findBy(key).getDescription(), key.getHour(), key.getMinute(),
                        attendance.getValue().getState());
                sj.add(historyFormat);
                continue;
            }
            String historyAbsenceFormat = String.format("%02d월 %02d일 %s --:-- (%s)", key.getMonthValue(),
                    key.getDayOfMonth(), Calender.findBy(key).getDescription(), attendance.getValue().getState());
