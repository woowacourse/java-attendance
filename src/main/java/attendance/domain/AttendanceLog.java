...
public class AttendanceLog {
    ...
    public Attendance registerAttendance(LocalDateTime attendanceDateTime) {
        ...
        attendanceLog.stream()
            .filter(attendance1 -> attendance1.checkAttendanceDateTimeEqual(attendanceDateTime.toLocalDate()))
            .findFirst()
    ...
    public List<Attendance> checkAttendancesRecord() {
        return attendanceLog.stream().filter(attendance -> attendance.getAttendanceDate().isBefore(LocalDate.of(2024,12,LocalDate.now().getDayOfMonth())))
            .toList();
    }
