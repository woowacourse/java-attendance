...
class CrewTest {
    ...
    void nullNameThrowException() {
        assertThatIllegalArgumentException().isThrownBy(() -> Crew.of(null)).withMessageContaining("[ERROR]");
    }

    @Test
    @DisplayName("이름이 공백이면 예외 발생")
    void blankNameThrowException() {
        assertThatIllegalArgumentException().isThrownBy(() -> Crew.of(" ")).withMessageContaining("[ERROR]");
    }
    ...
    void underNameLengthTwoThrowException() {
        String name = "한";
        assertThatIllegalArgumentException().isThrownBy(() -> Crew.of(name)).withMessageContaining("[ERROR]");
    }
