...
- [X] 사용자로부터 실행할 기능 번호를 입력받는다.
  - [X] 실행할 수 없는 기능 번호를 받는 경우, 재입력을 요청한다.
- [X] 입력받은 기능을 수행한 후 초기 화면으로 돌아간다.
- [X] 프로그램은 사용자가 종료할 때까지 종료되지 않으며, `Q`가 입력되면 종료된다.

### 1. 출석 확인 기능

- [X] 닉네임을 입력받는다.
  - [X] 데이터 목록에 닉네임이 존재하지 않는 경우, 예외 메세지를 출력한 후 초기화면으로 돌아간다.
- [X] 등교 시간을 입력받는다.
  - [X] HH:mm 형식이 아닐 경우 재입력 받는다.
- [X] 출석 후 출석 기록을 출력한다.(출석 후 출석 기록을 확인할 수 있다.)
...
- [X] 닉네임을 입력하면 전날까지의 크루 출석 기록을 확인할 수 있다.
  ...
  - [X] 전날까지의 출석 기록을 출력한다.
  - [X] 출석, 지각, 결석 횟수를 출력한다.
  - [X] 경고, 면담, 제적 대상자일 경우 안내 메시지를 출력한다.
- [X] 등교하지 않아 출석 기록이 없는 날은 결석으로 간주한다.

### 4. 제적 위험자 확인

- [X] **전날**까지의 크루 출석 기록을 바탕으로 제적 위험자를 출력한다.
  - [X] 출석 기록은 해당 월의 출석 기록만 이용한다.
- [X] 출석 기록이 없는 경우, 결석으로 간주한다.
