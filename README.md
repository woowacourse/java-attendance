...
- [x] 해당 요일의 시작 시각으로부터 5분 초과는 지각, 30분 초과는 결석으로 간주한다.
...
- [x] 프로그램은 사용자가 종료할 때까지 종료되지 않으며, 해당 기능을 수행한 후 초기 화면으로 돌아간다.
    - [x] 사용자 입력 후 예외가 발생한 경우, 메세지를 출력하고 다시 재입력 받는다.
...
- [x] 출석, 지각, 결석 횟수를 종합하여 확인할 수 있다.
- [x] 제적 위험자에 해당할 경우 제적 위험자 여부도 확인할 수 있다.
