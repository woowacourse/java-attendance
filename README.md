# java-attendance

출석 미션

## 기능 요구 사항

### 입력 기능

- [x] 기능 선택 입력
- [x] 출석/수정 크루 닉네임 입력
- [x] 등교/변경 시간 입력
- [x] 닐짜(일) 입력

### 핵심 기능

- [x] 기능을 선택하여 실행할 수 있다.
- [x] 프로그램은 사용자가 종료할 때까지 종료되지 않으며, 해당 기능을 수행한 후 초기 화면으로 돌아간다.
- [ ] 출석 확인
    - [x] 닉네임과 등교시간으로 출석 정보를 저장한다.
        - [x] 출석을 하면 출석부에 반영된다.
        - [x] 출석을 하면 출석 상태를 알 수 있다.
            - [x] 이미 출석한 경우, 다시 출석할 수 없으며 수정 기능을 이용하도록 안내한다.
        - [x] 교육 시간은 월요일은 13:00~18:00, 화요일~금요일은 10:00~18:00이다.
            - [x] 해당 요일의 시작 시각으로부터 5분 초과는 지각으로 간주한다.
            - [x] 해당 요일의 시작 시각으로부터 30분 초과는 결석으로 간주한다.
- [x] 출석 수정
    - [x] 출석 확인을 수정하려면 닉네임, 수정하려는 날짜, 등교 시간을 입력하여 기록을 수정할 수 있다.
    - [x] 출석을 수정하면 출석부에 반영된다.
- [x] 크루별 출석 기록 확인
    - [x] 닉네임을 입력하면 전날까지의 크루 출석 기록을 확인할 수 있다.
    - [x] 등교하지 않아 출석 기록이 없는 날은 결석으로 간주한다.
- [x] 제적 위험자 확인
    - [x] 전날까지의 크루 출석 기록을 바탕으로 제적 위험자를 파악한다.
    - [ ] 대상 항목별 정렬 순서는 지각을 결석으로 간주하여 내림차순한다.
    - [ ] 출석 상태가 같으면 닉네임으로 오름차순 정렬한다.
- [x] 누적 지각 및 결석 횟수에 따라 경고 또는 면담을 시행한다.
    - [x] 지각 3회는 결석 1회로 간주한다.
    - [x] 결석 횟수가 5회를 초과할 때 제적을 시행한다.

### 출력 기능

- [x] 기능 선택 안내 출력
- [x] 출석 후 출석 기록 출력
- [x] 출석 수정 후 변경 전과 변경 후의 출석 기록 출력
- [x] 전날까지의 크루 출석 기록 출력
- [x] 제적 위험자 정보 출력

### 파일 입출력 기능

- [x] 파일을 읽어와 저장한다.

### 예외 상황

- [x] 주말 및 공휴일에 출석 하는 경우
- [x] 파일의 구분자가 잘못된 경우
- [x] 파일의 경로가 잘못된 경우
- [x] 등록되지 않는 닉네임을 입력한 경우
- [x] 캠퍼스 운영시간 외에 출석 하는 경우
- [x] 존재하지 않는 날짜를 입력한 경우
- [x] 잘못된 파일인 경우
