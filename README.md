...
- [x]  2024년 12월 한 달 동안 시범적으로 최소한의 기능을 갖추도록 개발  
  → 추후 다른 월에도 사용하도록 확장성 고려
- [x]  코치가 사용하는 시스템

## 2. 출석 관리 규칙 및 시스템 설계 정책

- [x]  크루가 캠퍼스에 들어온 후 시스템에 출석 데이터가 저장된 시간을 기준으로 출석 확인
- [x]  24시간 형식만 사용
- [x]  캠퍼스 운영 시간
    - [x]  매일 08:00 ~ 23:00
- [x]  교육시간
    ...
    - [x]  지각: 해당 요일의 시작 시각으로부터 5분 초과
    - [x]  결석: 해당 요일의 시작 시각으로부터 30분 초과 & 등교하지 않아 출석 기록이 없는 날
- [x]  누적 지각 및 결석 횟수에 따라 경고 또는 면담 시행
    - [x]  지각 3회 = 결석 1회
    - [x]  경고 대상자: 결석 2회 이상
    - [x]  면담 대상자: 결석 3회 이상
    - [x]  제적 대상자: 결석 5회 초과
- [x]  프로그램은 사용자가 종료할 때까지 종료되지 않으며, 해당 기능을 수행한 후 초기 화면으로 이동
...
- [x]  수정하려는 날짜에 주말 또는 공휴일을 입력하면 수정할 수 없다.
...
- [x]  수정하려는 시간에 캠퍼스 운영 시간이 아닌 시간을 입력하면 수정할 수 없다.
...
- [x]  대상 항목별 정렬 순서는 지각을 결석으로 간주하여 내림차순한다. (결석 횟수가 많은 사람부터 출력)
- [x]  출석 상태가 같으면 닉네임으로 오름차순 정렬한다.(ㄱ → ㅎ 순으로 출력)
