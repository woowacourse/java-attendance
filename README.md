...
- [x] 프로그램 실행 시 src/main/resources/attendances.csv에서 필요한 정보를 불러온다.
- [x] 옵션을 입력받는다.
...
- [x] 옵션 1: 출석 확인
    - [x] 닉네임과 등교 시간을 입력하여 출석할 수 있다.
    - [x] 출석 후 출석 기록(월, 일, 요일, 시간, 출석 타입)을 반환한다.
    - [x] 이미 출석한 경우, 중복 출석이 불가하며 수정 기능을 이용하도록 안내한다.
    - [x] 주말에 출석을 시도하면 출석 불가를 안내한다.
- [x] 옵션 2: 출석 수정
    - [x] 출석 기록을 수정하려면 닉네임, 수정할 날짜, 변경할 등교 시간을 입력해야 한다.
    - [x] 닉네임과 날짜 기준으로 기존 출석 기록을 조회하여 수정한다.
    - [x] 수정 후, 변경 전 출석 기록(월, 일, 요일, 시간, 출석 타입)과 변경 후 출석 기록(시간, 출석 타입)을 가져온다.
- [x] 옵션 3: 크루별 출석 기록 확인
    - [x] 특정 크루의 출석 기록을 조회할 수 있다.
        - [x] 주말 및 공휴일을 제외한 전날까지의 출석 기록을 가져온다.
        - [x] 한 달간의 출석 기록(월, 일, 요일, 시간(없을 수 있음), 출석 타입)을 가져온다.
    - [x] 특정 크루의 출석 통계를 조회할 수 있다.
        - [x] 주말 및 공휴일을 제외한 전날까지의 출석, 지각, 결석, 총합을 가져온다.
        - [x] 출석 통계를 기준으로 제적 대상 여부를 판별한다.
- [x] 옵션 4: 제적 위험자 확인
    - [x] 전날까지의 출석 기록을 바탕으로 제적 위험자를 파악한다.
    - [x] 제적 위험자는 제적 대상자, 면담 대상자, 경고 대상자 순으로 출력한다.
        - [x] 정렬 기준: 지각을 결석으로 간주하여 내림차순 정렬
        - [x] 출석 상태가 동일할 경우, 닉네임 오름차순 정렬
    - [x] 각 위험자에 대해 닉네임, 결석 횟수, 지각 횟수, 제적 상태(제적/면담/경고) 정보를 가져온다.
