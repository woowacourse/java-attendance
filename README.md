...
- [x] 출석 수정
  - [x] 출석 확인을 수정하려면 닉네임, 수정하려는 날짜, 등교 시간을 입력한다.
  - [x] 수정하려는 날짜에 해당하는 출석 일시를 수정한다.
...
- [x] 크루별 출석 기록 확인
  - [x] 닉네임을 입력한다.
  - [x] 존재하는 크루인 지 확인한다.
  - [x] 전날까지의 크루 출석 기록을 반환한다.
  - [x] 등교하지 않아 출석 기록이 없는 날은 결석으로 간주한다.
  - [x] 출석 기록과 결석 기록을 출력한다.
  ```
  닉네임을 입력해 주세요.
  빙티
...
- [x] 제적 위험자 확인
  ...
  - [x] 제적 위험자는 제적, 면담, 경고 대상자순으로 출력하며, 지각을 결석으로 간주하여 내림차순한다.
  - [x] 출석 상태가 같으면 닉네임으로 오름차순 정렬한다.
  - [x] 누적 지각 및 결석 횟수에 따라 패널티를 부여한다.
