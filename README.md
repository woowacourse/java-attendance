...
- [x] 해당 요일의 교육 시작 시각으로부터 5분 초과는 지각으로 간주한다.
- [x] 해당 요일의 교육 시작 시각으로부터 30분 초과는 결석으로 간주한다.
...
```
제적 위험자 조회 결과
- 빙티: 결석 3회, 지각 4회 (면담) // 결석 3회, 지각 2회 (면담)
- 이든: 결석 2회, 지각 5회 (면담) // 결석 9회, 지각 1회 (제적)
- 빙봉: 결석 1회, 지각 6회 (면담) // 결석 9회, 지각 1회 (제적)
- 쿠키: 결석 2회, 지각 3회 (면담) // 결석 9회, 지각 1회 (제적)
- 짱수: 결석 0회, 지각 6회 (경고) // 결석 10회 (제적)
```

## 입출력 요구 사항
### 입력
- [x] 자세한 입력 예시는 실행 결과 예시를 참고하며, 프로그램을 시작하면 src/main/resources/attendances.csv를 통해 구현에 필요한 정보를 조회한다.
...
### 커스텀 요구사항 (추후 시간 남을 시 개발고려)
- [ ] 캠퍼스 운영 시간(08:00~23:00)이 아닐 때 출석하면 안내 메시지를 띄운다.
