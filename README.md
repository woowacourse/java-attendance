# java-attendance

출석 미션을 구현합니다.

## 출석 확인

- [x] 닉네임과 등교 시간을 입력하면 출석할 수 있다.
- [x] 출석 후 출석 기록을 확인할 수 있다.
- [x] 이미 출석한 경우, 다시 출석할 수 없으며 수정 기능을 이용하도록 안내한다.
- [x] 기록이 없는 닉네임을 입력하면 예외를 발생시킨다.
    - `[ERROR] 등록되지 않은 닉네임입니다.`
- [x] 등교일이 아닐 경우(주말, 공휴일) 예외를 발생시킨다.
    - `[ERROR] 12월 14일 토요일은 등교일이 아닙니다.`
- [x] 시간 입력 형식이 잘못된 경우 예외를 발생시킨다.
    - `[ERROR] 시간은 24시간 형식으로 입력해 주세요.`
- [x] 이미 출석을 하였는데 다시 출석 확인을 하는 경우 예외를 발생시킨다.
    - `[ERROR] 이미 출석을 확인하였습니다. 필요한 경우 수정 기능을 이용해 주세요.`

출력 예시:

```declarative
닉네임을 입력해 주세요.
이든
등교 시간을 입력해 주세요.
09:59

12월 05일 화요일 09:59 (출석)
```

## 출석 수정

- [x] 출석 확인을 수정하려면 닉네임, 수정하려는 날짜, 등교 시간을 입력하여 기록을 수정할 수 있다.
- [x] 수정 후에는 변경 전과 변경 후의 출석 기록을 확인할 수 있다.
- [x] 기록이 없는 닉네임을 입력하면 예외를 발생시킨다.
    - `[ERROR] 등록되지 않은 닉네임입니다.`
- [x] 출석 기록이 없는 날짜를 입력한 경우 예외를 발생시킨다.
    - `[ERROR] 출석 기록이 없는 날짜는 수정할 수 없습니다.`
- [x] 시간 입력 형식이 잘못된 경우 예외를 발생시킨다.
    - `[ERROR] 시간은 24시간 형식으로 입력해 주세요.`
- [x] 날짜 입력 형식이 잘못된 경우 예외를 발생시킨다.
    - `[ERROR] 숫자를 입력해 주세요.`
    - `[ERROR] 1~31 사이의 숫자를 입력해주세요.`

출력 예시:

```declarative
출석을 수정하려는 크루의 닉네임을 입력해 주세요.
빙티
수정하려는 날짜(일)를 입력해 주세요.
3
언제로 변경하겠습니까?
09:58

12월 03일 화요일 10:07 (지각) -> 09:58 (출석) 수정 완료!
```

## 크루별 출석 기록 확인

- [x] 닉네임을 입력하면 전날까지의 크루 출석 기록을 확인할 수 있다.
- [x] 출석 기록 목록을 출력한다.
- [x] 출결 현황을 출력한다. (출석, 지각, 결석)
- [x] 면담 대상자 여부를 출력한다.
- [x] 기록이 없는 닉네임을 입력하면 예외를 발생시킨다.
    - `[ERROR] 등록되지 않은 닉네임입니다.`

```declarative
닉네임을 입력해 주세요.
빙티

이번 달 빙티의 출석 기록입니다.

12월 02일 월요일 13:00 (출석)
12월 03일 화요일 10:07 (지각)
12월 04일 수요일 10:02 (출석)
12월 05일 목요일 10:06 (지각)
12월 06일 금요일 10:01 (출석)
12월 09일 월요일 --:-- (결석)
12월 10일 화요일 10:03 (출석)
12월 11일 수요일 --:-- (결석)
12월 12일 목요일 --:-- (결석)
12월 13일 금요일 10:02 (출석)

출석: 5회
지각: 2회
결석: 3회

면담 대상자입니다.
```

## 제적 위험자 확인

- [x] 전날까지의 크루 출석 기록을 바탕으로 제적 위험자를 파악한다.
- [x] 제적 위험자는 제적 대상자, 면담 대상자, 경고 대상자순으로 출력한다.
    - 대상 항목별 정렬 순서는 지각을 결석으로 간주하여 내림차순한다.
    - 출석 상태가 같으면 닉네임으로 오름차순 정렬한다.

```declarative
제적 위험자 조회 결과
- 빙티: 결석 3회, 지각 4회 (면담)
- 이든: 결석 2회, 지각 5회 (면담)
- 빙봉: 결석 1회, 지각 6회 (면담)
- 쿠키: 결석 2회, 지각 3회 (면담)
- 짱수: 결석 0회, 지각 6회 (경고)
```

## 메뉴 기능

- [x] 날짜와 메뉴 리스트를 안내한다.
- [x] 사용자가 입력한 값에 따라 해당 기능 실행한다.
- [x] 종료를 선택하면 프로그램을 종료한다.
- [x] 예외가 발생하면 초기 화면으로 돌아간다.

## 출석 관리 기능

- [x] 프로그램 시작 시 출석 기록 파일에서 출석 기록을 불러온다.
- [x] 입력한 출석 시간에 따라 출석, 지각 또는 결석으로 처리한다.
    - 교육 시간은 월요일은 13:00-18:00, 화요일-금요일은 10:00-18:00이다.
        - 해당 요일의 시작 시각으로부터 5분 초과는 지각으로 간주한다.
        - 해당 요일의 시작 시각으로부터 30분 초과는 결석으로 간주한다.
        - 등교하지 않아 출석 기록이 없는 날은 결석으로 간주한다.
- [x] 주말 및 공휴일에는 출석을 받지 않는다.
- [x] 등교 시간이 캠퍼스 운영 시간이 아닌 경우 예외를 발생시킨다
    - `[ERROR] 캠퍼스 운영 시간에만 출석이 가능합니다.`

## 제적 위험자 관리 기능

- [x] 누적 지각 및 결석 횟수에 따라 제적 위험자로 관리한다.
    - 지각 3회는 결석 1회로 간주한다.
        - 경고 대상자: 결석 2회 이상
        - 면담 대상자: 결석 3회 이상
        - 제적 대상자: 결석 5회 초과
